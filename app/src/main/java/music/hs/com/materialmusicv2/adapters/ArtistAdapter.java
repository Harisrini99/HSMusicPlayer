...
import music.hs.com.materialmusicv2.activities.ArtistActivity;
import music.hs.com.materialmusicv2.adapters.diffcallbacks.ArtistDiffCallback;
...
import static music.hs.com.materialmusicv2.utils.bitmaputils.BitmapUtils.lightenOrDarkenColorUntilTheyAreContrasty;
...
import static music.hs.com.materialmusicv2.utils.queryutils.QueryUtils.getSongsOfArtist;
...
public class ArtistAdapter extends RecyclerView.Adapter<ArtistAdapter.ViewHolder> implements FastScrollRecyclerView.SectionedAdapter {
    ...
    public ArtistAdapter(ArrayList<Artist> artists, Activity activity) {
        try {
            ...
            this.artistGrid = HSMusicApplication.getInstance().getPreferenceUtils().getArtistGrid();
            this.artistItemStyle = HSMusicApplication.getInstance().getPreferenceUtils().getArtistItemStyle();
            int imageType = HSMusicApplication.getInstance().getPreferenceUtils().getImageType();
            if (activity != null) {
                if (artistGrid == 1) {
    ...
    public ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
       // View itemView = LayoutInflater.from(parent.getContext()).inflate(artistGrid == 1 ? ((artistItemStyle == 0 || artistItemStyle == 4) ? R.layout.artist_item_gridsize_1 : R.layout.artist_item_gridsize_1_card) : (artistItemStyle == 0 ? R.layout.artist_item : (artistItemStyle == 4) ? R.layout.artist_item_circle : R.layout.artist_item_card), parent, false);
        View itemView = LayoutInflater.from(parent.getContext()).inflate(R.layout.artist_item_card,parent,false);
    ...
    private void colorViewHolder(ViewHolder holder, PaletteBitmap resource) {
        if (resource != null) {
            switch (artistItemStyle) {
                case 0: {
                    setTextColor(textColorPrimary, holder.artistName);
                ...
                case 3: {
                    ...
                    setTextColor(resource.foregroundColor, holder.artistName);
