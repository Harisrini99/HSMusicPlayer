package music.hs.com.materialmusicv2.utils.gaplessfadingmediaplayer;
...
import music.hs.com.materialmusicv2.utils.gaplessfadingmediaplayer.fadingmediaplayer.FadingMediaPlayer;
...
public class GaplessFadingMediaPlayer {
    ...
    private int getNextPosition(int position) {
        if (position + 1 >= HSMusicApplication.getInstance().getPlayingQueueManager().getNumberOfSongsInQueue()) {
            return 0;
        } else {
            return position + 1;
        }
    ...
    private Song getNextSong(int position) {
        if (position + 1 >= HSMusicApplication.getInstance().getPlayingQueueManager().getNumberOfSongsInQueue()) {
            return HSMusicApplication.getInstance().getPlayingQueueManager().getSongs().get(0);
        } else {
            return HSMusicApplication.getInstance().getPlayingQueueManager().getSongs().get(position + 1);
        }
    }
    ...
    private Song getCurrentSong(int position) {
        try {
            return HSMusicApplication.getInstance().getPlayingQueueManager().getSongs().get(position);
        } catch (Exception e) {
            e.printStackTrace();
            return null;
        }
