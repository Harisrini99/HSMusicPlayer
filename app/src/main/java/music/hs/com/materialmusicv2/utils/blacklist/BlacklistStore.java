package music.hs.com.materialmusicv2.utils.blacklist;
...
public class BlacklistStore extends SQLiteOpenHelper {
    ...
    public static synchronized BlacklistStore getInstance(@NonNull final Context context) {
        if (sInstance == null) {
            ...
            if (!HSMusicApplication.getInstance().getPreferenceUtils().getInitializedBlacklist()) {
                ...
                HSMusicApplication.getInstance().getPreferenceUtils().setInitializedBlacklist();
            }
        }
        return sInstance;
    }
