...
import static music.hs.com.materialmusicv2.utils.queryutils.QueryUtils.getSongsOfAlbum;
...
public class AlbumAdapter extends RecyclerView.Adapter<AlbumAdapter.ViewHolder> implements FastScrollRecyclerView.SectionedAdapter {
    ...
    public AlbumAdapter(ArrayList<Album> albums, Activity activity) {
        try {
            ...
            this.albumGrid = HSMusicApplication.getInstance().getPreferenceUtils().getAlbumGrid();
            this.albumItemStyle = HSMusicApplication.getInstance().getPreferenceUtils().getAlbumItemStyle();
    ...
    public ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        View itemView = LayoutInflater.from(parent.getContext()).inflate(R.layout.album_item_card,parent,false);
    ...
    private void colorViewHolder(ViewHolder holder, PaletteBitmap resource) {
        if (resource != null) {
            switch (albumItemStyle) {
                case 0: {
                    setTextColor(textColorPrimary, holder.albumName);
