...
public class QueueWidgetProvider extends AppWidgetProvider {
    ...
    public static void scroll() {
        AsyncTask.execute(() -> {
            try {
                ComponentName componentNameQueueWidget = new ComponentName(HSMusicApplication.getInstance().getApplicationContext(), QueueWidgetProvider.class);
                AppWidgetManager appWidgetManager = AppWidgetManager.getInstance(HSMusicApplication.getInstance().getApplicationContext());
                int[] appWidgetIds = appWidgetManager.getAppWidgetIds(componentNameQueueWidget);
                for (int appWidgetId : appWidgetIds) {
                    views.setScrollPosition(R.id.queue, HSMusicApplication.getInstance().getPlayingQueueManager().getSongPosition());
