package music.hs.com.materialmusicv2.service.notificationutils;
...
import music.hs.com.materialmusicv2.utils.drawableutils.DrawableUtils;
...
import static music.hs.com.materialmusicv2.utils.misc.Statics.CHANNEL_MEDIA_CONTROLS;
import static music.hs.com.materialmusicv2.utils.misc.Statics.NOTIFICATION_ID;
...
public class NotificationUtils implements SharedPreferences.OnSharedPreferenceChangeListener {
                ...
                try {
                    ...
                    if (!HSMusicApplication.getInstance().getPreferenceUtils().getUseMediaStyleNotification()) {
                        ...
                        notificationBuilder.setStyle(new androidx.media.app.NotificationCompat.MediaStyle()
                                ...
                                .setMediaSession(HSMusicApplication.getInstance().getMediaSessionUtils().getMediaSession().getSessionToken()));
                    }
    ...
    private synchronized void setQueue() {
        if (!HSMusicApplication.getInstance().getPreferenceUtils().getUseMediaStyleNotification()) {
            ...
            int songPosition = HSMusicApplication.getInstance().getPlayingQueueManager().getSongPosition();
            if (songs != null && !songs.isEmpty() && HSMusicApplication.getInstance().getPreferenceUtils().getShowQueueInNotification()) {
