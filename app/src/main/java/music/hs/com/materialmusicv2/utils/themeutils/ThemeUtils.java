package music.hs.com.materialmusicv2.utils.themeutils;
...
public class ThemeUtils {
    ...
    public static boolean isThemeDarkOrBlack() {
        try {
            return HSMusicApplication.getInstance().getPreferenceUtils().getTheme() != 0;
        } catch (Exception e) {
            e.printStackTrace();
        }
        return true;
    ...
    public static int getTheme(Context context) {
        if (context == null)
        {
            return R.style.AppTheme_NoActionBar_Light_Theme6;
        }
        int theme = HSMusicApplication.getInstance().getPreferenceUtils().getTheme();
        int accentColor = HSMusicApplication.getInstance().getPreferenceUtils().getAccentColor() + 1;
        try {
